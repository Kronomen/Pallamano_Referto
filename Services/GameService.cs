...
public sealed class GameService : IAsyncDisposable
{
    ...
    public bool RecordShootoutShot(bool goal)
    {
        ...
        if (goal)
        {
            // I tiri di rigore finali NON sono reti di gioco: non modificano State.ScoreA/B.
            // Il referto li visualizza separatamente e li usa solo per determinare il risultato ai rigori.
            if (pending.Team == "A") State.ShootoutScoreA++;
            else State.ShootoutScoreB++;
        }
